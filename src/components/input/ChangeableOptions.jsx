...
export const ChangeableOptions = ({className, value, setCheckboxes, hides, checkboxes}) => {
   ...
   const defaultValue = 'dishwasherDefaultValue'
   const changeableOptions = [
      ...
      {name: changeableOption11, type: inputType, id: changeableOption11, hide: hides[changeableOption11], attribute: {checked: checkboxes[changeableOption11]}, 
         ...
         ], defaultValue: defaultValue, className: dishwasherClass, hide: false} },
   ...
   useEffect(() => {
      setValue(localOption1, value.dishwasherSize)
      // eslint-disable-next-line react-hooks/exhaustive-deps
   }, [value.dishwasherSize])

   return (
      <div className={`${className}__${section4}`}>
      {changeableOptions.map((option) => {
         if (!option.hide && option.bind) {
            return <MyInputWithSelect key={option.id} className={section4 + itemClass} option={option} value={value} onChange={change} register={register} />
         } else if (!option.hide && !option.bind) {
            return <MyInput key={option.id} className={section4 + itemClass} option={option}  onChange={change}  />
         } else {
            return ''
         }
      })}
